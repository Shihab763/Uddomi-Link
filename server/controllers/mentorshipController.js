...
const createOffer = async (req, res) => {
  try {
    ...
    // prevent duplicate pending offers (same NGO -> same BO)
...
const getMyOffers = async (req, res) => {
  try {
    ...
    const offers = await MentorshipOffer.find({ businessOwnerId: req.user._id, status: "pending" })
      .sort({ createdAt: -1 })
      .populate("ngoId", "name email roles profile");
 ...
 * Business owner: list accepted mentors (NGOs)
 */
const getMyMentors = async (req, res) => {
  try {
    ...
    // return only NGOs, but still include offerId (useful)
 ...
 * NGO: list accepted mentees (business owners)
 */
 ...
 * NGO creates action plan for a mentee
 */
const createPlan = async (req, res) => {
  try {
    ...
    // must be accepted mentorship relation
    ...
    if (!relation) {
      return res.status(403).json({ message: "Mentorship must be accepted before creating plans." });
    }
 ...
 * Optional query ?ngoId=xxx to view for a specific mentor
 */
 ...
 * NGO views plans they created for a specific mentee
 */
const getPlansForMentee = async (req, res) => {
  try {
    ...
    const plans = await MentorshipPlan.find({ ngoId: req.user._id, businessOwnerId: menteeId })
      .sort({ createdAt: -1 });
 ...
 * Business owner marks a plan complete
 */
 ...
 * NGO recommends a training to a mentee (accepted relation required)
 */
const createRecommendation = async (req, res) => {
  try {
    ...
    if (!relation) {
      return res.status(403).json({ message: "Mentorship must be accepted before recommending." });
    }
 ...
 * Business owner views recommendations for them
 * Optional query ?ngoId=xxx to view for a specific mentor
 */
 ...
 * NGO views recs they created for a specific mentee
 */
const getRecommendationsForMentee = async (req, res) => {
  try {
    ...
    const recs = await MentorshipRecommendation.find({ ngoId: req.user._id, businessOwnerId: menteeId })
      .sort({ createdAt: -1 })
      .populate("trainingId", "title videoUrl tags level language thumbnailUrl");
 ...
 * Business owner marks recommendation complete
 */
